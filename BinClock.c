...
int main(void){
	...
	for (;;){
		...
		//Write your logic here
		hours = hexCompensation(wiringPiI2CWriteReg8(RTC, HOUR_REGISTER);
		minutes = hexCompensation(wiringPiI2CWriteReg8(RTC, MIN_REGISTER);
		seconds = hexCompensation(wiringPiI2CWriteReg8(RTC, SEC_REGISTER);
...
void hourInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		hours = decCompensation((hours+1)%24);
...
void minInc(void){
	...
	if (interruptTime - lastInterruptTime>200){
		...
		minutes = decCompensation((minutes+1)%60);
		//Write minutes back to the RTC
		wiringPiI2CWriteReg8(RTC, MIN_REGISTER, minutes);
	}
	lastInterruptTime = interruptTime;
}
